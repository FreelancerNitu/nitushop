...
if ( ! function_exists( 'nitu_shop_setup' ) ) :
	/**
	 ...
	 */
	function nitu_shop_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on Algori Shop, use a find and replace
		 * to change 'nitu-shop' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'nitu-shop', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'nitu-shop' ),
			'menu-2' => esc_html__( 'Footer', 'nitu-shop' ),
		) );
		 ...
		 */
		 add_filter('get_custom_logo', 'nitu_shop_change_logo_class');
		 
		 function nitu_shop_change_logo_class ($html){
		 ...
		 */
		 function nitu_shop_display_logo(){
		 ...
		 */
		 function nitu_shop_comments_format($comment, $args, $depth){
			  ...
			  echo '<li '; comment_class('x'); echo' id="li-comment-'; comment_ID(); echo '">';
				...
				echo '<div class="message">
						...
						echo '</div>
							...
							/* Comment Message */
							$comment_message = __( 'Your comment is awaiting moderation.', 'nitu-shop' );
		 ...
		 */
		 function nitu_shop_get_the_comments_navigation( $args = array() ) {
			$navigation = '';
		 
			// Are there comments to navigate through?
			if ( get_comment_pages_count() > 1 ) {
				$args = wp_parse_args( $args, array(
					'prev_text'          => __( 'Older comments', 'nitu-shop' ),
					'next_text'          => __( 'Newer comments', 'nitu-shop' ),
					'screen_reader_text' => __( 'Comments navigation', 'nitu-shop' ),
				) );
		...
		function nitu_shop_get_the_single_posts_navigation( ) {
			...
			if($next_link = get_next_post()){ // Display next post link if it exists
				echo '<div class="navigation pull-right">';
					$next_btn_title =  __( 'Next', 'nitu-shop' );
		 ...
		 */
		function nitu_shop_get_the_posts_navigation( $args = array() ) {
			$navigation = '';
		 
			// Don't print empty markup if there's only one page.
			if ( $GLOBALS['wp_query']->max_num_pages > 1 ) {
				$args = wp_parse_args( $args, array(
					'prev_text'          => __( 'Older posts', 'nitu-shop' ),
					'next_text'          => __( 'Newer posts', 'nitu-shop' ),
					'screen_reader_text' => __( 'Posts navigation', 'nitu-shop' ),
				) );
...
add_action( 'after_setup_theme', 'nitu_shop_setup' );
 ...
 */
function nitu_shop_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'nitu_shop_content_width', 640 );
}
add_action( 'after_setup_theme', 'nitu_shop_content_width', 0 );
 ...
 */
function nitu_shop_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'nitu-shop' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'nitu-shop' ),
...
add_action( 'widgets_init', 'nitu_shop_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function nitu_shop_scripts() {
	...
	wp_enqueue_style( 'nitu-shop-style', get_stylesheet_uri() );
	
	wp_style_add_data( 'nitu-shop-style', 'rtl', 'replace' );

	wp_enqueue_style( 'nitu-shop-blue', get_template_directory_uri() . '/style/css/color/blue.css', array(), '20180131', 'all' );

	// Add Fonts
	wp_enqueue_style( 'nitu-shop-google-fonts', 'http://fonts.googleapis.com/css?family=Raleway:400,300,500,600,700,800,900' );
	...
	wp_enqueue_script( 'nitu-shop-scripts', get_template_directory_uri() . '/style/js/scripts.js', array( 'jquery' ), '20190318', true );

	wp_enqueue_script( 'nitu-shop-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'nitu-shop-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'nitu_shop_scripts' );

/**
 * Enqueue fonts for gutenberg.
 */
function nitu_shop_block_editor_fonts(){
	wp_enqueue_style( 'nitu-shop-block-editor-fonts', 'http://fonts.googleapis.com/css?family=Raleway:400,300,500,600,700,800,900' );
}
add_action( 'enqueue_block_editor_assets', 'nitu_shop_block_editor_fonts' );
 ...
 */
 function nitu_shop_excerpt_more( $more ) {
...
add_filter('excerpt_more', 'nitu_shop_excerpt_more');

 /**
 * Customize Algori Shop Search form input and submit button .
 */
 function nitu_shop_get_search_form($form) {
 ...
 add_filter('get_search_form', 'nitu_shop_get_search_form');
 
/**
 * Algori Shop walker nav class.
 */
class nitu_shop_Walker_Nav_Primary extends Walker_Nav_menu {
